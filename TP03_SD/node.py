...
class ClusterNode:
    def __init__(self, node_id: int, host: str, port: int, peer_info: List[Dict]):
        ...
        # Configuração do Cluster Store
        self.stores = [
            {"id": 0, "host": "store0", "port": 6000},
            {"id": 1, "host": "store1", "port": 6000},
            {"id": 2, "host": "store2", "port": 6000},
        ]
        ...
        if m_type == "client_request":
            ...
            with self.lock:
                ...
                if self.state == "HELD" or (self.state == "WANTED" and not is_higher_priority):
                    ...
                    self.log(f"--- FILA DE ESPERA: {self.deferred_replies} (OK adiado para Nó {remote_id}) ---")
                else:
    ...
    def process_client_queue(self):
        ...
        while self.running:
            ...
            success = self.access_store(client_id)

            if success:
                self.log(">>> Escrita replicada com sucesso (quórum atingido).")
            else:
                self.log(">>> Falha: quórum não atingido!")
    ...
    def access_store(self, client_id):
        success_count = 0
        ...
        for store in self.stores:
            try:
                ...
                self.log(f"Store {store['id']} não respondeu.")

        # Quórum: pelo menos 2 de 3
